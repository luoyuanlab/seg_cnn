""" 
This file was slighted adapted from https://github.com/yoonkim/CNN_sentence by Yoon Kim
yluo-05/01/2016 add LogisticRegression.preds() 
yluo-05/06/2016 changed LogisticRegression.errors()
yluo-12/26/2016 changed downsample.max_pool_2d to pool.pool_2d (theano dev)
"""
...
class MLPDropout(object):
    ...
    def __init__(self,rng,input,layer_sizes,dropout_rates,activations,use_bias=True):
        ...
        for n_in, n_out in self.weight_matrix_sizes[:-1]:
            ...
            # path through the graph. yluo: why don't use next_dropout_layer directly?
        ...
        # Grab all the parameters together. yluo comprehension? 
        self.params = [ param for layer in self.dropout_layers for param in layer.params ]

    def predict(self, new_data):
        next_layer_input = new_data
        # yluo: I think you need to think it symbolically? 
...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2), non_linear="tanh"):
        ...
        """
        # is 1 in conv_net_sentence.py, what does it mean if > 1?
        ...
        if self.non_linear=="none" or self.non_linear=="relu":
            ...
            self.W = theano.shared(numpy.asarray(rng.uniform(low=-W_bound, high=W_bound, size=filter_shape),
                dtype=theano.config.floatX),borrow=True,name="W_conv")   
        b_values = numpy.zeros((filter_shape[0],), dtype=theano.config.floatX) # b is of length number of hidden units
